...
class Follow(object):
    def __init__(self, config):
        ...
        if not isinstance(user_id_list, list):
            ...
            user_id_list, user_name_list = self.get_user_list(user_id_list)
        ...
        self.file_dir = os.path.split(os.path.realpath('__file__'))[0]
        if not os.path.isdir(self.file_dir):
            os.makedirs(self.file_dir)
        self.csv_path = self.file_dir + os.sep + 'user_follow_list.csv'
        self.txt_path = self.file_dir + os.sep + 'user_id_list.txt'
    ...
    def write_to_txt(self):
        with open(self.file_dir + os.sep + 'user_id_list.txt', 'ab') as f:
            for user in self.follow_list:
                f.write((user['uri'] + ' ' + user['nickname'] + '\n').encode(
                    sys.stdout.encoding))

    def write_to_csv(self):
        with open(self.file_dir + os.sep + 'user_follow_list.csv',
                  'a',
                  newline='') as f:
            writer = csv.writer(f)
            for user in self.follow_list:
            ...
            for line in lines:
                ...
                if len(info) == 2 and info[0].isdigit():
                    ...
                    if user_id not in user_id_list:
                        ...
                        user_name_list.append(info[1])

        return user_id_list, user_name_list

    def initialize_info(self, i):
        """初始化爬虫信息"""
        self.follow_list = []
        self.user_id = self.user_id_list[i]
        self.user_name = self.user_name_list[i]

    def start(self):
        """运行爬虫"""
        j = 1
        while j <= self.level:
            print('爬取第{}层微博粉丝'.format(j))
            print('*' * 100)
            try:
                for i in range(len(self.user_id_list)):
                    self.initialize_info(i)
                    print('*' * 100)
